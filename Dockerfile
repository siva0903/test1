...
RUN mkdir -p                  "${BAMBOO_HOME}/lib" "${BAMBOO_INSTALL}"\
    ...
    && curl -Ls                "https://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-java-8.0.25.tar.gz" | tar -xz --directory "${BAMBOO_INSTALL}/lib" --strip-components=1 --no-same-owner "mysql-connector-java-5.1.40/mysql-connector-java-5.1.40-bin.jar" \
...
# Add kubecfg binary
ADD https://github.com/bitnami/kubecfg/releases/download/v0.20.0/kubecfg-linux-amd64
